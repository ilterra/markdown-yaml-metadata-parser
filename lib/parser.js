...
/**
 * @typedef {Object} ParserDependencies
 * @property {Function} detectNewline Detect the new line char given a string.
 * @property {Function} parseYaml Parse a string as YAML document.
 */
...
/**
 * @param {ParserDependencies} parserDependencies.
 ...
 */
const Parser = ({ detectNewline, parseYaml }) => (text) => {
  let METADATA_START
  let METADATA_END
  ...
  const parseMetadata = () => {
    const parseResult = result.metadata ? parseYaml(result.metadata) : {}
    result = {
      ...result,
